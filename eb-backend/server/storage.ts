...
import { db, queryWithTimeout, resetDatabaseConnection } from "./db";
...
export class DatabaseStorage implements IStorage {
  ...
  async getProjects(): Promise<Project[]> {
    try {
      ...
      // Check if we should try again after a connection reset
  ...
  async createProject(project: InsertProject): Promise<Project> {
    try {
      const [newProject] = await queryWithTimeout(
